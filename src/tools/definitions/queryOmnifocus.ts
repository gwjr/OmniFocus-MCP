...
export const schema = z.object({
  ...
  where: coerceJson('where', z.any().optional().describe(
    `Expression tree for filtering using compact syntax. ${ALL_OPS.length} operations: ${ALL_OPS.join(', ')}. Use {opName: [args]} syntax (e.g. {contains: [{var: "name"}, "review"]}). Dates: {date: "YYYY-MM-DD"}, {offset: {date: "now", days: -3}}, {var: "now"}. Date ranges: {between: [{var: "dueDate"}, {var: "now"}, {offset: {date: "now", days: 7}}]}. Tags: {contains: [{var: "tags"}, "tagName"]}. Container scoping: {container: ["project"|"folder"|"tag", expr]} filters tasks by their containing project/folder/tag (the sub-expression matches against the container entity, e.g. {container: ["project", {eq: [{var: "name"}, "MyProject"]}]}). For simple project name matching, prefer: {contains: [{var: "projectName"}, "text"]}. Semantic search: {similar: ["query"]} or {similar: ["query", 60]} with optional 0-100 threshold; composes with and/or; default limit 20; auto-injects similarity field. Null handling: comparisons (gt, lt, gte, lte) with null return false; use isNull/isNotNull to test.

FIELD REFERENCE — valid {var: "..."} names:
Tasks: id(str), name(str), flagged(bool), dueDate(date?), deferDate(date?), plannedDate(date?), effectiveDueDate(date?), effectiveDeferDate(date?), effectivePlannedDate(date?), completionDate(date?), modificationDate(date), creationDate(date), estimatedMinutes(num?), blocked(bool), completed(bool), dropped(bool), inInbox(bool), sequential(bool), hasChildren(bool), projectName(str?), projectId(str?), parentId(str?), tags(array), note(str), taskStatus(enum: Next|DueSoon|Overdue|Blocked|Completed|Dropped), now(date).
Projects: id, name, status(enum: Active|Done|OnHold|Dropped), flagged, completed, dueDate, deferDate, effectiveDueDate, effectiveDeferDate, completionDate, modificationDate, creationDate, estimatedMinutes, sequential, taskCount, activeTaskCount, folderId, folderName, note.
Folders: id, name, hidden, parentFolderId, projectCount, status(enum: Active|Dropped).
Tags: id, name, allowsNextAction, hidden, effectivelyHidden, availableTaskCount, remainingTaskCount, parentId, parentName, note.`
  )),
