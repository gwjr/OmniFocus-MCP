...
import { compileEventPlan } from '../dist/tools/query/executionUnits/orchestrator.js';
import type { StrategyNode, BulkScan } from '../dist/tools/query/strategy.js';
...
// ══════════════════════════════════════════════════════════════════════════
// compileEventPlan smoke tests (full sync pipeline: StrategyNode → units)
// ══════════════════════════════════════════════════════════════════════════

describe('compileEventPlan', () => {

  it('compiles a simple BulkScan into targeted plan and units', () => {
    const scan: BulkScan = {
      kind: 'BulkScan',
      entity: 'tasks',
      columns: ['id', 'name'],
    ...
    const { targeted, units } = compileEventPlan(scan);
    assert.ok(targeted.nodes.length > 0, 'should produce at least one targeted node');
    assert.ok(units.length > 0, 'should produce at least one execution unit');
    assert.ok(
      units.some(u => u.runtime === 'jxa'),
      'BulkScan should produce at least one JXA unit',
    );
  });

  it('produces a node unit for Zip in a multi-column BulkScan', () => {
    const scan: BulkScan = {
      kind: 'BulkScan',
      entity: 'tasks',
    ...
    const { units } = compileEventPlan(scan);
    const nodeUnit = units.find(u => u.runtime === 'node');
    assert.ok(nodeUnit, 'multi-column BulkScan should produce a node unit (for Zip)');
  });

  it('compiles a BulkScan with Filter child', () => {
    ...
    const { targeted, units } = compileEventPlan(tree);
    assert.ok(targeted.nodes.length >= 2, 'should have nodes for scan + filter');
    // Filter should be in a node unit
    const nodeUnits = units.filter(u => u.runtime === 'node');
    assert.ok(nodeUnits.length > 0, 'Filter should produce a node unit');
  });

  it('compiles a Sort → Limit → BulkScan tree', () => {
    const tree: StrategyNode = {
      kind: 'Limit',
      source: {
        kind: 'Sort',
    ...
    const { units } = compileEventPlan(tree);
    assert.ok(units.length >= 1, 'should have at least one unit');
    // Sort and Limit should be in node units
    const nodeUnits = units.filter(u => u.runtime === 'node');
    assert.ok(nodeUnits.length > 0, 'Sort/Limit should run in node');
  });

  it('all units have valid structure', () => {
    const scan: BulkScan = {
      kind: 'BulkScan',
      entity: 'tags',
      columns: ['id', 'name'],
    ...
    for (const unit of units) {
      assert.ok(['jxa', 'omniJS', 'node'].includes(unit.runtime), `valid runtime: ${unit.runtime}`);
      assert.ok(unit.nodes.length > 0, 'unit should have at least one node');
      assert.ok(typeof unit.result === 'number', 'result should be a Ref');
      assert.ok(unit.nodes.includes(unit.result), 'result ref should be in unit nodes');
      assert.ok(Array.isArray(unit.dependsOn), 'dependsOn should be an array');
      assert.ok(Array.isArray(unit.inputs), 'inputs should be an array');

      // All node refs should be valid indices in the targeted plan
      for (const ref of unit.nodes) {
        assert.ok(ref >= 0 && ref < targeted.nodes.length, `node ref ${ref} in bounds`);
      }
    }
  });
});
